    ...
    $(document).ready(function() {
		...
		$('#calendar<fmt:formatDate value="${date}" type="both" pattern="ddMMyyyy" />').fullCalendar({
            ...
            events: [
                ...
                {
                    title: "<c:if test="${not empty talk.schedule.room}"><span class='room'> ${talk.schedule.room.name} </span></c:if><c:if test="${talk.schedule.duree >= 60}">\n</c:if><span class='title'>${custo:formatchaine(talk.title)}</span>",
